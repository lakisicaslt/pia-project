    ...
    constructor() {
        ...
        this.changePassword = (req, res) => {
            ...
            if (hashedNewPassword == hashedNewPassword2) {
                user_1.default.findOne({ 'username': username, 'password': hashedOldPassword }).then(user => {
                    if (!user) {
        ...
        this.adminLogin = (req, res) => {
            ...
            let password = req.body.password;
            const hashedPassword = this.hashPassword(password);
            user_1.default.findOne({ 'username': username, 'password': hashedPassword }).then(data => {
                if ((data === null || data === void 0 ? void 0 : data.tip) == "Vlasnik" || (data === null || data === void 0 ? void 0 : data.tip) == "Dekorator") {
        ...
        this.getAllOwners = (req, res) => {
            let tip = "Vlasnik";
            user_1.default.find({ 'tip': tip }).then(data => {
                res.json(data);
            }).catch(err => {
                console.log(err);
            });
        };
        this.getAllOwnersNotActive = (req, res) => {
            let tip = "Vlasnik";
            user_1.default.find({ 'tip': tip, 'isActive': "false" }).then(data => {
                res.json(data);
            }).catch(err => {
                console.log(err);
            });
        ...
        this.getAllNotWorkingDecorators = (req, res) => {
            let tip = "Dekorator";
            user_1.default.find({ 'tip': tip, 'works': false }).then(data => {
                res.json(data);
            }).catch(err => {
                console.log(err);
            });
        };
        this.azurirajDohvati = (req, res) => {
            let username = req.body.username;
            user_1.default.findOne({ 'username': username }).then(user => {
                res.json(user);
            }).catch(err => {
